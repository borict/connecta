...
import connecta.message.exception.ApiErrorResponse;
...
public class OpenApiConfig {
    ...
    OpenAPI messageServiceOpenApi() {
        ...
        return new OpenAPI()
                .info(new Info()
                        .title("Connecta Message Service")
                        .description("""
                                Direct messages and 1:1 conversations.
                                Login via User Service POST /api/auth/login, copy the token,
                                then Authorize → Bearer token here.
                                WebSocket STOMP destinations are documented in the README, not in Swagger.
                                """)
                        .version("v1"))
