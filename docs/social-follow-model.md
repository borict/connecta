...
Feed (`GET /api/feed`) lives in Social: ACCEPTED following IDs + self → Post `by-authors`. Post down → empty page. Explore (`GET /api/explore`) is the inverse discovery feed: public authors the viewer does not ACCEPTED-follow → same Post `by-authors`; User or Post down → empty page.
